...
const AddList = (props) => {
    ...
    return (
        <div>
            ...
            {show && <div>
                <textarea value={list} onChange={(e)=>setlist(e.target.value)} className='p-1 w-full rounded-md border-2 bg-zinc-700 border-zinc-900' name="" id="" cols="30" rows="2" placeholder='Enter Description'></textarea>
            ...
            {!show && <button onClick={()=> setShow(!show)} className='flex p-1 w-full justify-center rounded items-center mt-1 hover:bg-gray-500 h-8'>
                <Plus size={16}></Plus> Add a New Task
            </button>}
            </div>
        </div>
    );
